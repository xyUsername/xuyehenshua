...
$res = query($sql);//先获取数据表单里的值；
$json = $res[0]["value"];//因为返回的是二维数组，所以要取到value这个键的值；
$data = json_decode($json);//把字符串转换为php变量
// 新数据此时还缺一个id， 现在的做法是把最大的id存储到 表格里面，每次新增从里面获取，并把最大id+1
$sqlMaxId = "SELECT o.value FROM options o WHERE o.`key` = 'max_nav_id'";//新数据ID为4；
$maxId = query($sqlMaxId);//查询得到这个id；
$maxId = $maxId[0]["value"];//取到id；
//让最大id+1作为新数据的id
$_POST["id"] = $maxId + 1;//如果它作为新的id，让它+1;
